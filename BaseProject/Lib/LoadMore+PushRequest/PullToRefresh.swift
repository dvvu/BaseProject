...
public class PullToRefreshView: UIView {
    ...
    private var loadMoreDirection: LoadMoreDirection = .Vertical
...
extension PullToRefreshView {
    ...
    private func handleScrollViewOffsetChange() {
        ...
        if isLoading {
            ...
            if loadMoreDirection == .Vertical {
                ...
                var insetTop = originalContentInset.left
                
                if -contentOffset.x > originalContentInset.left {
                    insetTop = -contentOffset.x
                }
                
                if insetTop > frame.size.width + originalContentInset.left {
                    insetTop = frame.size.width + originalContentInset.left
                }
                oldInset.left = insetTop
                
                scrollView.contentInset = oldInset
                insetTopDelta = originalContentInset.left - insetTop
                return
            }
        }
        ...
        if loadMoreDirection == .Vertical {
            ...
            if scrollView.contentOffset.x < -adjustedContentInset.left {
                if abs(scrollView.contentOffset.x) - adjustedContentInset.left > frame.size.width {
                    if !scrollView.isDragging {
                        isLoading = true
                        animator.pullToRefresh(self, stateDidChange: .loading)
                    } else {
    ...
    private func startAnimating() {
        ...
        if loadMoreDirection == .Vertical {
            ...
            let frameHeight = frame.size.width
            var contentOffset = CGPoint(x: -frameHeight, y: 0)
            if #available(iOS 11.0, *) {
                contentOffset.x = -scrollView.adjustedContentInset.left - frameHeight
            }
            UIView.animate(withDuration: 0.3, delay: 0, options: UIView.AnimationOptions(), animations: {
                self.scrollView.contentInset = UIEdgeInsets(top: 0, left: frameHeight + contentInset.left, bottom: 0, right: 0)
                self.scrollView.contentOffset = contentOffset
            }, completion: { finished in
                self.animator.pullToRefreshAnimationDidStart(self)
                self.action()
    ...
    private func stopAnimating() {
        if loadMoreDirection == .Vertical {
            ...
            UIView.animate(withDuration: 0.3, animations: {
                var oldInset = self.scrollView.contentInset
                oldInset.left = oldInset.left + self.insetTopDelta
                self.scrollView.contentInset = oldInset
            }, completion: { finished in
                self.animator.pullToRefreshAnimationDidEnd(self)
            })
