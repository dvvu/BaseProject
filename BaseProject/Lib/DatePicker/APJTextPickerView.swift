...
public protocol APJDatePickerViewDelegate: class {
...
open class APJTextPickerView: UITextField, UITextViewDelegate {
    ...
    public weak var datePickerDelegate:APJDatePickerViewDelegate?
...
//        picker.maximumDate = Calendar.current.date(byAdding: .day, value: -5840, to: Date())
...
//        components.year = -16
    ...
    private func _updateCurrentDate() {
        if _oldDateValue != currentDate {
            ...
            datePickerDelegate?.textPickerView(self, didSelectDate: currentDate)
            _updateDateText()
