...
//  Ebates
//
//  Created by Macbook on 10/17/18.
...
open class InfiniteCollectionView: UICollectionView {
    ...
    private var scrollDuration: Float = 5
...
private extension InfiniteCollectionView {
    ...
    private func configTimer() {
        if (timer != nil) {
            ...
            timer = Timer.scheduledTimer(timeInterval: TimeInterval(scrollDuration), target: self, selector: #selector(runTimedCode), userInfo: nil, repeats: true)
        }
    }
    
    @objc func runTimedCode() {
        
        if let numberOfItems = infiniteDataSource?.number(ofItems: self), numberOfItems > 0 {
            self.scrollRectToVisible(CGRect(x: contentOffset.x + itemWidth, y: contentOffset.y, width: itemWidth, height: self.frame.size.height), animated: true)
        }
    }
